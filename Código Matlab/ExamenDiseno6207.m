...
% ------------------------------------------------
% 2.4 Rodamiento fijo para el árbol principal
% ------------------------------------------------
% Se usa SKF 6207 porque fue el rodamiento seleccionado para la memoria,
% el CAD y los asientos del árbol principal.
bearingSKF.model    = "SKF 6207";
bearingSKF.bore_mm  = 35.0;
bearingSKF.width_mm = 17.0;

% Capacidades de catálogo SKF 6207
bearingSKF.C_kN     = 25.5;   % Capacidad dinámica básica [kN]
bearingSKF.C0_kN    = 15.3;   % Capacidad estática básica [kN]

kN_to_lbf = 224.809;

bearingSKF.bore_in  = bearingSKF.bore_mm/25.4;
bearingSKF.width_in = bearingSKF.width_mm/25.4;
bearingSKF.C_lbf    = bearingSKF.C_kN*kN_to_lbf;
bearingSKF.C0_lbf   = bearingSKF.C0_kN*kN_to_lbf;

% ------------------------------------------------
% 2.5 Posiciones axiales del arbol principal
...
% ------------------------------------------------
% 2.6 Posiciones axiales del eje de salida
...
% Aquí se verifica directamente el rodamiento SKF 6207 en los apoyos A y B.

d_journal_min_in = bearingSKF.bore_in;

bearingA = evaluateFixedBallBearing(shaftMain.FrA, 0, kin.n_main, ...
    inp.Lcoj_h, bearingSKF);

bearingB = evaluateFixedBallBearing(shaftMain.FrB, 0, kin.n_main, ...
    inp.Lcoj_h, bearingSKF);
...
function bearing = evaluateFixedBallBearing(Fr_lbf, Fa_lbf, n_rpm, life_h_req, bearingSKF)
% Verifica un rodamiento rígido de bolas fijo.
% No selecciona otro modelo: evalúa el rodamiento indicado en bearingSKF.
%
% Para carga axial nula:
%   Fe = Fr
%
% Vida modificada por confiabilidad:
%   Lna = a1*L10
%
% Para 99 % de confiabilidad se toma:
%   a1 = 0.21

a_life = 3;       % Exponente para rodamientos de bolas
a1 = 0.21;        % Factor de confiabilidad aproximado para 99 %

if Fa_lbf == 0
    Fe_lbf = Fr_lbf;
else
    % Si existiera carga axial, habría que completar X, Y y e
    % con el catálogo SKF. Para este diseño se toma Fa = 0.
    X = 1.0;
    Y = 0.0;
    Fe_lbf = X*Fr_lbf + Y*Fa_lbf;
end

% Vida requerida modificada para confiabilidad del 99 %
Lreq_mrev_99 = 60*n_rpm*life_h_req/1e6;

% Vida L10 requerida equivalente
L10_req_mrev = Lreq_mrev_99/a1;

% Capacidad dinámica requerida
Creq_lbf = Fe_lbf*(L10_req_mrev)^(1/a_life);

% Vida real con el rodamiento fijo
L10_mrev = (bearingSKF.C_lbf/Fe_lbf)^a_life;
L99_mrev = a1*L10_mrev;
life_h_99 = L99_mrev*1e6/(60*n_rpm);

bearing.model = bearingSKF.model;
bearing.bore_mm = bearingSKF.bore_mm;
bearing.bore_in = bearingSKF.bore_in;
bearing.width_mm = bearingSKF.width_mm;
bearing.width_in = bearingSKF.width_in;
bearing.C_lbf = bearingSKF.C_lbf;
bearing.C0_lbf = bearingSKF.C0_lbf;
bearing.Fr_lbf = Fr_lbf;
bearing.Fa_lbf = Fa_lbf;
bearing.Fe_lbf = Fe_lbf;
bearing.Creq_lbf = Creq_lbf;
